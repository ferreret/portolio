...
`/cv-en.pdf`, `/cv-es.pdf`, and `/profile.png` are served from the `public/` directory. The CV PDFs are generated by visiting `/cv` in the browser (which renders `components/CVView.tsx` from `data/cv/{en,es}.ts`), toggling language, and printing to PDF (Ctrl+P → Save as PDF, A4, "Background graphics" enabled). The `Download CV` button in `HomeView` links to `/cv-${language}.pdf`.
